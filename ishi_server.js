...
io.on("connection", socket => {
    socket.on("user_connected", user => {
        const roomId = `${ROOM_PREFIX}` + `${user.slotId}`;
        socket.join(roomId)
        console.log("user user_connected " + roomId);
    });

    socket.on("prescription_received", prescription => {
        const roomId = `${ROOM_PREFIX}` + `${prescription.slotId}`;
        socket.to(roomId).emit("broadcast_prescription", prescription);
    });

    socket.on("receive_report", report => {
        const roomId = `${ROOM_PREFIX}` + `${report.slotId}`;
        socket.to(roomId).emit("broadcast_report", report);
    });
...
server.listen(port, () => {
    console.log(`Ishi socket server is running on  ${port}`)
})